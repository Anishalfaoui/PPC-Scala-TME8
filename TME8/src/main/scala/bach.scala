...
def receive = {
    case "START" => {
      ...
      // question 5 / figure (6) - Canon de Bac
      play(figure_bach)
    }
}
...
//Question 1
  ...
  // voix 1 : do - mi - sol - silence - re
  val voix1_ex = Sequential(List(
    Note(60, 1000, 100), Note(64, 1000, 100), Note(67, 1000, 100), Rest(500), Note(62, 500, 100)
  ))

  // voix 2 : re (en deux temps) - sol - fa
  val voix2_ex = Sequential(List(
    Note(62, 2000, 100), Note(67, 1000, 100), Note(65, 1000, 100)
  ))

  // Les deux voix en parallele
  val figure_1 = Parallel(List(voix1_ex, voix2_ex))
...
// Question 4 - Figure 2 : Identite ; Transposition (12 tons) ; Retrograde ; Miroir (centre = 60)

  // HAUT : Do, Mi-Sol, Silence, La
  val fig_top = Sequential(List(
    Note(60, 500, 100), Note(64, 250, 100), Note(67, 250, 100), Rest(500), Note(69, 500, 100)
  ))
  // BAS : Do, Sol, Fa
  val fig_bot = Sequential(List(
    Note(48, 1000, 100), Note(55, 500, 100), Note(53, 500, 100)
  ))
  val figure_originale = Parallel(List(fig_top, fig_bot))

  // Transposition 12 demitons
  val figure_transposee = transpose(figure_originale, 12)

  // Retrograde
  val figure_retrograde = retrograde(figure_originale)

  // Miroir centre = 60
  val figure_miroir = mirror(figure_originale, 60)

  // Les 4 transformations en sequence
  val figure_2 = Sequential(List(
    figure_originale,      // Identite
    Rest(500),
    figure_transposee,     // Transposition 12 tons
    Rest(500),
    figure_retrograde,     // Retrograde
    Rest(500),
    figure_miroir          // Miroircentre = 60
  ))
...
// Question 4 - Figure 3 : canon (repeat figure_1 3) 1000
  val figure_3 = canon(repeat(figure_1, 3), 1000)
...
// make obj en parallele avec lui meme avec un decalage de n ms.
  def canon (obj:ObjectMusical, n:Int):ObjectMusical =
    Parallel(List(obj, Sequential(List(Rest(n), copy(obj)))))
...
// Question 5 - Canon Bach (figure 6)

  def canon_Bach ():ObjectMusical = {
    // voix 1 : roi
    val v1 = Sequential((0 to 5).map(i => transpose(voix1, i * 2)).toList)

    // Voix 2 : canon
    val v2 = Sequential((0 to 5).map(i => transpose(voix2, i * 2)).toList)

    // Voix 3 : voix 2 transposee decalee(2000 ms)
    val v3 = Sequential(Rest(2000) :: (0 to 5).map(i => transpose(voix2, 7 + i * 2)).toList)

    // final
    Parallel(List(v1, v2, v3))
  }

  val figure_bach = canon_Bach()
}
